...
Use the `linear_graphql` bash command (available in PATH during symphony sessions) or
the `linear_graphql` client tool if running inside a Codex app-server session.
